...
const cardController = {
  ...
  //     const collection = user.allCollections.find(
  ...
  //           (card) => card.id.toString() === update.id
  //         );
  //         if (cardIndex !== -1) {
  //           const card = collection.cards[cardIndex];
  //           update.collectionId = collection?._id;
  //           console.log(
  //             `UPDATING CARD: ${card?.name} in collection ${collection}`
  //           );
  ...
  //     const updatedCollection = user.allCollections.find(
  //       (c) => c._id.toString() === collectionId
  //     );
  ...
  //       throw new Error(
  //         "Card data is incomplete. Both id and name are required."
  //       );
  //     }
  ...
  //             card[key] =
  //               key === "quantity" ? Math.max(1, update[key]) : update[key];
  //           }
  //           card.deck = deck._id;
  ...
  //           await CardInDeck.findByIdAndUpdate(currentCard._id, {
  //             quantity: currentCard.quantity,
  //           });
  //         }
  //       }
  ...
  patchCard: async (cardId, cardData) => {
    ...
    try {
      const user = await User.findById(cardData.userId).populate(
        "allCollections"
      );
      if (!user) {
      ...
      await Promise.all(
        user.allCollections.map((collection) => collection.save())
      );
