...
function logResponseData(response) {
  const dataToLog = Array.isArray(response.data)
...
const updateDailyCollectionPriceHistoryData = async (userId) => {
  try {
    ...
    throw new Error(
      `Error updating daily collection price history: ${error.message}`
    );
  }
};
  ...
  const updatePromises = selectedList.map(async (card) => {
    ...
    if (cardInCollection) {
      const newChartDataEntry = createChartDataEntry(
        cardInCollection.latestPrice.num
      );
  ...
  try {
    if (!userId || !Array.isArray(selectedList)) {
      throw new CustomError(
        "Invalid inputs provided to scheduleCheckCardPrices.",
        400
      );
    }
...
const scheduleUpdateDailyCollectionPriceHistoryDataJob = (io, selectedList) => {
  ...
  setupCronJob(
    io,
    () => updateDailyCollectionPriceHistoryData(selectedList),
    cronSchedule
  );
};

function addToEmittedResponses(response, eventType) {
  emittedResponses.push({
    index: responseIndex++,
    eventType,
    timestamp: new Date(),
    response,
  });
  getIO().emit("EMITTED_RESPONSES", {
    message: "YES",
    data: emittedResponses.slice(-25),
  });
}
...
const filterOldEventTypes = () => {
  ...
  emittedResponses = emittedResponses.filter(
    (response) => response.timestamp > oneHourAgo
  );
};
