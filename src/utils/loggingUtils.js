...
const { LOG_TYPES, ERROR_TYPES } = require("../../src/configs/constants");
const { logToSpecializedLogger } = require("../middleware/loggers/infoLogger");
...
function determineLogType(data, data2) {
  ...
  if (Array.isArray(data)) {
    ...
    if (
      data.length > 0 &&
      data.every((item) =>
        Object.prototype.hasOwnProperty.call(item, "priceHistory")
      )
    ) {
  ...
  if (data && typeof data === "object") {
    ...
    if (
      Object.prototype.hasOwnProperty.call(data, "totalPrice") &&
...
const processCard = (data) => {
  ...
  try {
    ...
    cards.forEach((card, index) => {
      ...
      const statusColor =
        priceChange > 0 ? "green" : priceChange < 0 ? "red" : "grey";
      const status =
        priceChange > 0
          ? "increased"
          : priceChange < 0
  ...
  try {
    ...
    collections?.forEach((collection, index) => {
      ...
      collection?.cards?.forEach((card, cardIndex) => {
        try {
          const latestPrice = parseFloat(card?.latestPrice?.num ?? 0)?.toFixed(
            2
          );
          const lastSavedPrice = parseFloat(
            card?.lastSavedPrice?.num ?? 0
          )?.toFixed(2);
          ...
          console.error(
            `Error processing card in collection ${cardIndex}:`,
            cardError
          );
...
function logSingleCollection(collection) {
  ...
  logCollection(
    collection.updatedCollection ? collection.updatedCollection : collection
  );
...
function logData(data, data2) {
  ...
  switch (logType) {
    ...
    case LOG_TYPES.CARDS:
      ...
      logContent +=
        "[MULTIPLE CARDS] ".yellow + `${new Date().toISOString()}\n`;
      console.log(logContent);
    ...
    case LOG_TYPES.COLLECTION:
      ...
      logContent +=
        "[SINGLE COLLECTION] ".yellow + `${new Date().toISOString()}\n`;
      console.log(logContent);
    ...
    case LOG_TYPES.COLLECTIONS:
      ...
      logContent +=
        "[MULTIPLE COLLECTION] ".yellow + `${new Date().toISOString()}\n`;
...
function formatErrorLog(errorLog) {
  ...
  formattedLog += `| [Data]: ${
    errorLog.debugInfo.data
      ? JSON.stringify(errorLog.debugInfo.data, circularReplacer(), 2)
      : "N/A"
  }\n`;
...
const logPriceChange = (
  ...
  try {
    if (!card) {
      throw new Error(
        "[logPriceChange] -----> Invalid card data provided for logging.".red
      );
    }
    ...
    if (changeStatus === "CHANGE") {
      messageBody =
        priceDifference > 0
          ? `increased by $${priceDifference.toFixed(2)} (${percentChange.toFixed(2)}%)`
              .green
          : priceDifference < 0
